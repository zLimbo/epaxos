...
func (r *Replica) run() {
	...
	for !r.Shutdown {
		...
		case propose := <-onOffProposeChan:
			...
			dlog.Printf("Proposal with op %d, size: %v", propose.Command.Op, GetSize(propose))
		...
		case prepareS := <-r.prepareChan:
			...
			dlog.Printf("Received Prepare from replica %d, for instance %d, size: %v", prepare.LeaderId, prepare.Instance, GetSize(prepare))
			r.handlePrepare(prepare)

		case acceptS := <-r.acceptChan:
			accept := acceptS.(*paxosproto.Accept)
			//got an Accept message
			dlog.Printf("Received Accept from replica %d, for instance %d, size: %v", accept.LeaderId, accept.Instance, GetSize(acceptS))
			r.handleAccept(accept)

		case commitS := <-r.commitChan:
			commit := commitS.(*paxosproto.Commit)
			//got a Commit message
			dlog.Printf("Received Commit from replica %d, for instance %d, size: %v", commit.LeaderId, commit.Instance, GetSize(commitS))
		...
		case prepareReplyS := <-r.prepareReplyChan:
			...
			//got a Prepare reply
			dlog.Printf("Received PrepareReply for instance %d, size: %v", prepareReply.Instance, GetSize(prepareReplyS))
			r.handlePrepareReply(prepareReply)

		case acceptReplyS := <-r.acceptReplyChan:
			acceptReply := acceptReplyS.(*paxosproto.AcceptReply)
			//got an Accept reply
			dlog.Printf("Received AcceptReply for instance %d, size: %v", acceptReply.Instance, GetSize(acceptReplyS))
...
func (r *Replica) handlePropose(propose *genericsmr.Propose) {
	if !r.IsLeader {
		...
		r.ReplyProposeTS(preply, propose.ReplyWriter)
		return
	}

	dlog.Printf("propose.size = %d, propose.CommandId = %d", GetSize(propose), propose.CommandId)
...
func (r *Replica) handlePrepare(prepare *paxosproto.Prepare) {
	...
	var prepareReply *paxosproto.PrepareReply
	...
	if inst == nil {
		...
		prepareReply = &paxosproto.PrepareReply{prepare.Instance, ok, r.defaultBallot, make([]state.Command, 0)}
	} else {
		ok := TRUE
		if prepare.Ballot < inst.ballot {
			ok = FALSE
		}
		prepareReply = &paxosproto.PrepareReply{prepare.Instance, ok, inst.ballot, inst.cmds}
	}

	r.replyPrepare(prepare.LeaderId, prepareReply)
		...
		if inst.lb.acceptOKs+1 > r.N>>1 {
			...
			if inst.lb.clientProposals != nil && !r.Dreply {
				...
				for i := 0; i < len(inst.cmds); i++ {
					...
					r.ReplyProposeTS(propreply, inst.lb.clientProposals[i].ReplyWriter)
				}
			}
			...
			if r.instanceSpace[i].cmds != nil {
				...
				for j := 0; j < len(inst.cmds); j++ {
					...
					if r.Dreply && inst.lb != nil && inst.lb.clientProposals != nil {
						...
						r.ReplyProposeTS(propreply, inst.lb.clientProposals[j].ReplyWriter)
					}
				}
				i++
				executed = true
