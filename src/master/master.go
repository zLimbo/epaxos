...
var numNodes *int = flag.Int("N", 4, "Number of replicas. Defaults to 4.")
...
func main() {
	...
	log.Printf("listen addr: %s", l.Addr().String())
	if err != nil {
...
func (master *Master) Register(args *masterproto.RegisterArgs, reply *masterproto.RegisterReply) error {
	...
	if index == nlen {
		...
		log.Printf("peer register, index: %d, addr: %s", index, addrPort)
	}
